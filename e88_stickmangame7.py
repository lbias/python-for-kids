       ...
       # check if y is greater than 0, meaning falling
        ...
        for sprite in self.game.sprites:
            ...
            if left and self.x < 0 and collided_left(co, sprite_co):
                ...
                left = False
                # check if the sprite that stick figure has colided with has and endgame variable that is set to True
                if sprite.endgame:
                    # if so, end the game
...
class DoorSprite(Sprite):
    ...
    def __init__(self, game, photo_image, x, y, width, height):
        # call __init__ of parent class Sprite
        Sprite.__init__(self, game)
        # save the parameter photo_image using an object variable with the same name
        self.photo_image = photo_image
        # create a display image, save the identifying number using the object variable image
        self.image = game.canvas.create_image(x, y, image=self.photo_image, anchor='nw')
        # set the coordinates of DoorSprite
        self.coordinates = Coords(x, y, x + (width / 2), y + height)
        # when the stick figure reaches the door, the game should end
        self.endgame = True
...
# add variable door to the list of sprites stored in the game object
